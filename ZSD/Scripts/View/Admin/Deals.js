$(document).ready(function () {
    ...
    $(document).on('click', '.editview', function () {
        if ($.session.get("deal_id") != '' || $.session.get("deal_id") != null || $.session.get("deal_id") == undefined) {
            ...
            window.location.href = '/Admin/Dealdetails';
        }
    });
});
...
function InsNewDeal() {
    var parm = {
        ...
        "Licensing_Subscription": $("#ddllicense option:selected").val(),
                ...
                columns: [
                    ...
                    //},
                    {
                        data: 'Deal_Id', sWidth: '100px', render: function (Deal_Id, type, row) {
                            return '<a href="javascript:void(0);"  class="editview"  name="' + Deal_Id + '"> <i class="fa fa-eye icon-ser"> </i> DL0000'+ row.Deal_Id + '</a>';
                    ...
                    { data: 'City' },
                    {
                    ...
                    },
                     {
                        ...
                        render: function (data, type, row) {
                            ...
                            let html = '';
                            if (data == 0) {
                                html += '<select onchange="changeStatus(this);" data-id="' + row['Deal_Id'] + '">';
                                if (status < 1) {
...
function changeStatus(elmnt) {
    let Dealid = $(elmnt).attr('data-id');
    let Approval_status = $(elmnt).val(); 
    var parm = {
        "Deal_Id": Dealid,
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: apiurl + 'api/CommonApi/changedealstatus',
        success: function (data) {
